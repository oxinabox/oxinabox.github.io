...
You can see did `@time @time ...` and two number were displayed.
...
It's worth looking at this later because it turns on invalidations (see below) made the REPL have to recompile a bunch of display code.
...
Note also that in the above timing I had already run precompilation.
Which seems fairest.
...
Recall I said most of julia compilation  isn't even that useful for plotting, since making it run fast isn't a priority (but loading fast is)
